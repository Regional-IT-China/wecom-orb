...
PostToWecom() {
    ...
    WECOM_SENT_RESPONSE=$(curl -s -f -X POST -H 'Content-type: application/json' --data "$SLACK_MSG_BODY" https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=$WECOM_ACCESS_TOKEN)
